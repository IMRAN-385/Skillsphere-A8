...
const NavPage = () => {
  ...
  const avatarUrl = user?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'U')}&background=dc2626&color=fff`;
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-[#1e201e] border border-[#697565]/30 rounded-2xl shadow-2xl py-2 z-50">
                    <div className="px-4 py-3 border-b border-[#697565]/20">
                      ...
                      <p className="text-xs text-[#697565]">{user.email}</p>
                    </div>
                    <Link href="/profile" className="block px-4 py-2 text-sm text-[#ecdfcc] hover:bg-[#3C3D37] hover:text-red-400 transition">
                      My Profile
                    </Link>
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-[#3C3D37] transition border-t border-[#697565]/20 mt-1"
                    >
                      Logout
                    </button>
                  </div>
          ...
          <div className="md:hidden border-t border-[#697565]/20 bg-[#1e201e] py-4 space-y-1">
            ...
            {!user ? (
              <div className="pt-2 space-y-2">
                <Link href="/LoginPage" className="block px-4 py-3 text-center text-white border border-[#697565]/40 rounded-lg">
                  Login
                </Link>
                <Link href="/Register" className="block px-4 py-3 text-center bg-red-600 text-white font-semibold rounded-lg">
                  Register
                </Link>
              </div>
            ) : (
